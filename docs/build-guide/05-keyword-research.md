...
1. **Map domains to Semrush markets.** Add `markets[]` to Domains: `locale` (select from the domain's active locales) + `semrushDatabase` (Semrush regional database code, e.g. `us`, `uk`, `de`). A `beforeValidate` hook requires exactly one market per active locale. Update the seed script, regenerate types, and add a migration.
   **Verify:** saving Alpha with `en` → `us` and `de` → `de` succeeds, removing the `de` market while `de` is active fails validation, and a migration exists.
...
3. **Store research runs.** New collection `keyword-research-runs` (logged-in users only):
   - Fields: `domain`, `locale`, `semrushDatabase`, `seedKeywords[]`, `reports[]` (which Semrush reports ran), `rows` (JSON array of typed rows, deduped by normalized keyword), `unitsUsed`, `status` (`running` | `complete` | `failed`), `error`.
   - `src/lib/keywords/research.ts` → `runResearch({ domainId, locale, seedKeywords, limitPerReport })`. It reuses a `complete` run with identical inputs from the last 30 days instead of calling Semrush again, unless `force: true`.
...
4. **Cluster keywords.** `src/lib/keywords/cluster.ts` → `clusterKeywords(runId)`:
   ...
   - Input: the run's rows (keyword, volume, KD, intent) and the domain name/niche.
...
5. **Detect cannibalization.** `src/lib/keywords/dedupe.ts` → `findConflicts(domainId, clusters)`. It flags a proposed cluster whose normalized primary keyword (lowercase, trimmed, singularized, stop-words removed) matches, or whose keyword set overlaps ≥ 50% with, an existing `keyword-clusters` doc for the same domain in any status, or a post's `meta.title` keyword. Other domains are ignored — separate domains are allowed to target the same topic (spec §4).
   **Verify:** `vitest` tests: "best hiking backpacks" conflicts with an existing "best hiking backpack" cluster on Alpha, the same cluster on Beta doesn't conflict, and a 60% keyword overlap is flagged.

6. **Extend and persist KeywordClusters.** Add `primaryKeyword`, `suggestedTemplate`, `researchRun` (relationship), and per-keyword `searchVolume`, `keywordDifficulty`, `cpc`, `intent`. `targetTemplate` defaults to `suggestedTemplate` but stays editable. `targetLocales` (from phase 03) defaults to the domain's active locales. `src/lib/keywords/saveClusters.ts` → `saveClusters({ runId, clusters, targetDomainId })` creates `unused` clusters and refuses any that `findConflicts` flags, unless `allowConflict: true` is passed per cluster. Regenerate types and add a migration.
   **Verify:** saving the step 4 output creates `unused` clusters linked to the run with metrics and `targetLocales` filled, a conflicting cluster is refused with a reason, and a migration exists.

7. **Add Server Actions for phase 06.** `src/lib/keywords/actions.ts` with `'use server'` exports `runResearchAction`, `clusterKeywordsAction`, `saveClustersAction`, and `updateClusterAction` (rename, edit keywords, change target template and target locales, only while `unused`). Each action:
...
8. **Run an end-to-end research flow.** Script `npm run research -- --domain <alphaId> --locale en --seeds "hiking backpack" --limit 20`: research → cluster → conflicts → save. Then generate one of the resulting clusters through phase 03's `npm run generate`.
   **Verify:** the script saves real clusters for Alpha from live Semrush data and reports units used. One saved cluster successfully produces a draft post through phase 03's pipeline. If Semrush access isn't available, say so and leave the box unchecked.
...
- [ ] **Step 1:** Domains map each active locale to one Semrush database, with validation and a migration
...
- [ ] **Step 5:** Cannibalization detection flags same-domain duplicates and overlaps but ignores other domains
- [ ] **Step 6:** Clusters persist as `unused` with metrics, suggested template, target locales, and run link; conflicts are refused unless allowed
- [ ] **Step 7:** Keyword Server Actions require an authenticated user, validate input, and refuse edits to non-`unused` clusters
- [ ] **Step 8:** A live research → cluster → save → generate flow produced a draft post from Semrush data
