...
3. **Store research runs.** New collection `keyword-research-runs` (logged-in users only):
   ...
   - `src/lib/keywords/research.ts` → `runResearch({ domainId, seedKeywords, limitPerReport })` queries the domain's `semrushDatabase`. It reuses a `complete` run with identical inputs (same database, seeds, and limits) from the last 30 days instead of calling Semrush again, unless `force: true`.
...
4. **Cluster keywords.** `src/lib/keywords/cluster.ts` → `clusterKeywords(runId)`:
   ...
   - Input: the run's rows (keyword, volume, KD, intent), the domain name/niche, and the domain `locale`. Cluster names and rationales are written in the domain's language so editors reviewing that domain read them naturally.
